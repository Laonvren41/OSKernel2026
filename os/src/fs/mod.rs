...
pub fn flush_preload() {
    ...
    let _ = test.write(UserBuffer::new(v));
}
...
pub fn map_dynamic_link_file<'a>(path: &'a str, prefix: &'a str) -> &'a str {
    ...
    if file_name == "ld-musl-riscv64-sf.so.1" {
        let fallback_path = "/musl/lib/libc.so";
        debug!("[map_dynamic] musl linker not found, falling back to libc.so: {}", fallback_path);
        if DYNAMIC_PATH.contains(fallback_path) {
